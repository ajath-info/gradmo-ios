...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let window = KeyboardDismissWindow(windowScene: windowScene)
        self.window = window

        let rootViewController: UIViewController
        if shouldRestoreLoggedInSession() {
            let storyboard = UIStoryboard(name: "Home", bundle: nil)
            let tabBarVC = storyboard.instantiateViewController(
                withIdentifier: "CustomTabBarController"
            ) as! CustomTabBarController
            tabBarVC.initialUserType = UserCache.getUserRole()
...
private extension SceneDelegate {
    func shouldRestoreLoggedInSession() -> Bool {
        let isLoggedIn = UserDefaults.standard.bool(forKey: LoginKeys.isLoggedIn)
        let token = UserCache1.authtoken()
        let userId = UserCache.userID()

        return isLoggedIn && !token.isEmpty && !userId.isEmpty
    }

    func hasSeenOnboarding() -> Bool {
        UserDefaults.standard.bool(forKey: LoginKeys.didSeeOnboarding)
    }
}
